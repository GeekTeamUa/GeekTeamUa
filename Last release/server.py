...
#    res3 = loaded_model.predict_classes(img_array)
    ...
    if (select == "2") :
        ...
        hpercent = (baseheight / float(img.size[1]))
        wsize = int((float(img.size[0]) * float(hpercent)))
        img = img.resize((27, 35), Image.ANTIALIAS)
        img_array = np.array(img)
        img_array = img_array.reshape(1, 27, 35, 1)
        ...
        # elif res_str=="":
        #     answ=""
           #return render_template("index.html", final_text="ua-mnist", img="A")

        return render_template("index.html", final_text = "ua-mnist",img=answ)
...
def train():
   ...
   #  from CNN_FOR_FUNC import SCRIPT_CONV_NEURAL_NETWORK
    ...
    #inputRange1 = request.form.get("input_range_1")
    #inputRange2 = request.form.get("input_range_2")
   ...
   # epochs = 1
    ...
    # batch_size = 128
    # epochs = 12
    # dataset = 1  # 1 - UaMnist, 2 - FashionMnist
    # activation_function = 'relu'

    #SCRIPT_CONV_NEURAL_NETWORK(batch_size, epochs, dataset, activation_function)
    ...
    return render_template("training.html", how =a)
